...
ASTAR_EVALUATOR_JAR_PATH = os.path.join(pt, "PlayAStar_Evaluate.jar")
...
class Simulator:
    def __init__(
        ...
        self.astar_evaluator_jar_path = ASTAR_EVALUATOR_JAR_PATH
    ...
    def astar_evaluate(self, render: bool = False):
        t = tempfile.NamedTemporaryFile(suffix=".txt", delete=False)
        save_level(self.level, t.name)
        print(f"Running Astar agent on level! -- {t.name}")
        render_str = "human" if render else "norender"
        result = subprocess.run(
            ["java", "-jar", self.astar_evaluator_jar_path, t.name, render_str, IMAGE_PATH],
            stdout=subprocess.PIPE,
